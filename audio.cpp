    ...
    AVCodecContext **input_codec_context)
{
    ...
    // Set up 1 channel beacuse mic camera usb has 1 (mono)
    av_dict_set(&options, "channels", "1", 0);

    // Open alsa input  (example: "hw:1,0")
    if ((error = avformat_open_input(input_format_context, filename, input_format, &options)) < 0) {
        fprintf(stderr, "Could not open ALSA device '%s' (error: %s)\n",
        filename, av_err2str_cpp(error));
        return error;
    }
    ...
    if ((error = avformat_find_stream_info(*input_format_context, NULL)) < 0) {
        ...
        av_err2str_cpp(error));
        avformat_close_input(input_format_context);
        return error;
    }

    // // Chỉ chấp nhận 1 stream
    ...
    // Find codec
    input_codec = avcodec_find_decoder((*input_format_context)->streams[0]->codecpar->codec_id);
        if (!input_codec) {
                              ...
                              int *data_present)
{
    ...
    if (*data_present &&
        ...
        log_packet_tb(&output_codec_context->time_base, &output_packet);
    }
