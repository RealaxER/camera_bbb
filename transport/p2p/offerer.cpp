...
int main(int argc, char **argv) {
	...
	pc->onLocalDescription([](rtc::Description description) {
		std::cout << "Local Description 1 (Paste this to the other peer):" << std::endl;
		std::cout << std::string(description) << std::endl;
	});

	pc->onLocalCandidate([](rtc::Candidate candidate) {
		std::cout << "Local Candidate 2 (Paste this to the other peer after the local description):"
		          << std::endl;
	...
	while (!exit) {
		std::cout
			...
			<< " 3: Enter remote description 2/"
		    << " 4: Enter remote candidate 2/"
		    << " 5: Send message /"
		    << " 6: Send message /"
		    << " 7: Print Connection Info *" << std::endl
		    << "[Command]: ";
		...
		// }
		case 4: {
			auto dc2 = pc->createDataChannel("file_transfer");

			dc2->onOpen([&]() { std::cout << "[DataChannel open: " << dc2->label() << "]" << std::endl; });

			dc2->onClosed([&]() { std::cout << "[DataChannel closed: " << dc2->label() << "]" << std::endl; });

			dc2->onMessage([](auto data) {
				if (std::holds_alternative<std::string>(data)) {
					std::cout << "[Received on file_transfer: " << std::get<std::string>(data) << "]" << std::endl;
				}
			});
			sleep(100);
			break;
